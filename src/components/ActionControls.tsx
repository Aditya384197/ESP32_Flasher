    ...
    <div className="space-y-4 pt-1">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900/70 border border-slate-800 rounded-xl p-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            id="btn-load-demo-bin"
            onClick={onLoadDemoBins}
            disabled={isFlashing}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-cyan-300 text-xs border border-slate-700 transition disabled:opacity-50"
            title="Load ready-made test ESP32 binaries"
          >
            <Sparkles className="w-3.5 h-3.5 text-amber-400" />
            <span>Load Demo Bins</span>
          </button>
      ...
      <div className="bg-slate-900/90 border border-slate-800/90 rounded-2xl p-4 sm:p-5 shadow-xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[11px] text-slate-400 mt-1.5 text-center">
              {!isConnected 
                ? '(Ready in Simulated mode or plug USB-OTG)' 
                : settings.eraseAndFlash 
