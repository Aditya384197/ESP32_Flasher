    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        ...
        <div className="p-4 sm:p-5 overflow-y-auto space-y-4 text-xs sm:text-sm">
          {/* Installed-app notice */}
          <div className="bg-cyan-950/30 border border-cyan-800/50 rounded-xl p-3 text-[11px] text-cyan-200/90 flex items-start gap-2">
            <AlertTriangle className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
            <p className="leading-relaxed">
              <strong>Installed app:</strong> the app talks to real hardware through a built-in native USB-serial bridge (not the browser's Web Serial API, which the embedded WebView doesn't support). Plug in the OTG cable, tap <strong>Connect</strong>, and allow the Android USB permission prompt when it appears — that's the whole flow inside the app. The steps below (chrome://flags, device picker) only apply if you're opening this app's web build directly in desktop Chrome/Edge instead of using the installed APK.
            </p>
          </div>
