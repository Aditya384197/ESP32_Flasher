...
export default function App() {
  ...
  // Connect to ESP32 (Hardware via WebSerial / Android OTG — real devices only)
  const handleConnectDevice = async () => {
    setIsConnecting(true);
    try {
      addLog('Connecting to device at ' + settings.baudRate + ' baud...', 'info');
      const dev = await flasherService.connect(settings.baudRate, addLog);
      setDeviceInfo(dev);
  ...
  const handleEraseFlash = async () => {
    ...
    // If not connected, prompt real hardware connection
  ...
  const handleFlash = async () => {
    ...
    if (activeSlots.length === 0) {
      addLog('Please select at least one .bin file to flash.', 'error');
      return;
    }

    // If not connected, request real hardware connection
    if (!deviceInfo.connected) {
